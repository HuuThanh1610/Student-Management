...
//                ? (List<Subject>) cacheSubject.getOrDefault("listSubject", new ArrayList<>()) : connectDB.SubjectListing();
            boolean commit = gradeDB.deleteGradeDatabase(Integer.parseInt(markID));
            if (commit) {
                ...
                json = gson.toJson(new GradeAPIResponse("Grade delete successfully", HttpServletResponse.SC_OK, gradeView));
            } else {
                json = gson.toJson(new GradeAPIResponse("Failed to delete grade.", HttpServletResponse.SC_BAD_REQUEST, null));
            }
            out.print(json);
            out.flush();
