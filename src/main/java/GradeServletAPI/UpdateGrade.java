...
public class UpdateGrade extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String markID = request.getParameter("markID").trim();
            String markOfSubject = request.getParameter("markOfSubject").trim();
            String reason = request.getParameter("reason").trim();
            Gson gson = new Gson();
            String json;
            if (!gradeDB.checkExistGradeByMarkID(Integer.parseInt(markID))) {
                out.print(new Gson().toJson(new GradeAPIResponse("Grade does not exist.", HttpServletResponse.SC_BAD_REQUEST, null)));
                out.flush();
                return;
            }
            int parsedMarkID;
            double parsedMarkOfSubject;
            try {
                parsedMarkID = Integer.parseInt(markID);
                parsedMarkOfSubject = Double.parseDouble(markOfSubject);
            } catch (NumberFormatException e) {
                out.print(new Gson().toJson(new GradeAPIResponse("Invalid mark format.", HttpServletResponse.SC_BAD_REQUEST, null)));
                return;
            }
...
//                    ? (List<Subject>) cacheSubject.getOrDefault("listSubject", new ArrayList<>()) : connectDB.SubjectListing();
            ...
            //new grade
            boolean commit = gradeDB.updateGradeDatabase(parsedMarkID, parsedMarkOfSubject, reason);
            if (commit) {
                ...
                json = gson.toJson(new GradeAPIResponse("Grade updated successfully", HttpServletResponse.SC_OK, gradeView));
            } else {
                json = gson.toJson(new GradeAPIResponse("Failed to update grade", HttpServletResponse.SC_BAD_REQUEST, null));
            }
            out.print(json);
            out.flush();
//                // clear data cache
