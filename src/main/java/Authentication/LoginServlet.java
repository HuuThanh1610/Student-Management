...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (authenDB.checkLogin(email, password)) {
                UserAccount userAccount = authenDB.getUserAccount(email);
                if (userAccount != null) {
                    //create session
...
//                    session.setAttribute("User", userAccount);
                ...
                json = gson.toJson(new UserAccountAPIResponse("Incorrect email or password.", HttpServletResponse.SC_BAD_REQUEST, null));
            }
            out.print(json);
            out.flush();
        }
