...
//                    ? (List<Teacher>) cacheClass.getOrDefault("listTeacher", new ArrayList<>()) : connectDB.TeacherListing();
            boolean classDelete = classBD.deleteClassDatabase(Integer.parseInt(classID));
            if (classDelete) {
                ...
                json = gson.toJson(new ClassAPIResponse("Class delete successfully", HttpServletResponse.SC_OK, studentClassView));
            } else {
                json = gson.toJson(new ClassAPIResponse("Failed to delete class.", HttpServletResponse.SC_BAD_REQUEST, null));
            }
            out.print(json);
            out.flush();
