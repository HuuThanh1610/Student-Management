...
public class UpdateClass extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String schyear = request.getParameter("shooclYear").trim();
            Gson gson = new Gson();
            String json;
            if (!classBD.checkExistClassbyClassID(Integer.parseInt(classID))) {
                out.print(new Gson().toJson(new ClassAPIResponse("Class does not exist.", HttpServletResponse.SC_BAD_REQUEST, null)));
                out.flush();
                return;
            }
...
//                ? (List<Teacher>) cacheClass.getOrDefault("listTeacher", new ArrayList<>()) : connectDB.TeacherListing();
            ...
            //new class
            StudentClass newStuClass = new StudentClass(Integer.parseInt(classID), className, schyear, Integer.parseInt(teacherID));
            boolean commit = classBD.updateClassDatabase(newStuClass);
            if (commit) {
                ...
                json = gson.toJson(new ClassAPIResponse("Class update successfully.", HttpServletResponse.SC_OK, studentClassView));
            } else {
                json = gson.toJson(new ClassAPIResponse("Failed to update Class.", HttpServletResponse.SC_BAD_REQUEST, null));
            }
            out.print(json);
            out.flush();
